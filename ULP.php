...
<option value="ULP.PHP?unit1=Unit Layanan Pelanggan (ULP) Binjai Barat" selected>Unit Layanan Pelanggan (ULP)</option>
...
<select name="unit1"  id="mySelect"onchange="myFunctioner()">
                                   <optgroup label="select unit">
                                       <?php
                                       $sqli = "SELECT * FROM unit WHERE unit2 <> unit1";
                                       $results = $conn->query($sqli);
                                       
                                       if ($results->num_rows > 0) {
                                           while($rows = $results->fetch_assoc()) {
                                            if (($rows['unit2'] != "Unit Pelaksana Pengatur Distribusi (UP2D) Sumatera Utara ")&&($rows['unit2'] != "Unit Pelaksana Proyek Ketenagalistrikan (UP2K) Provinsi Sumatera Utara")&&
                                            ($rows['unit2'] != "Unit Pelaksana Pelayanan Pelanggan (UP3) Nias")
                                            ){
                                       ?>
...
function myFunctioner() {
  ...
  window.location = 'ULP.php?unit1='+x;
}
</script>
        </div>
